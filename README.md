...
|--------|-----|--------------|
| **Сайт** | `http://ip_ALB/` | – |
| **Grafana** | `http://grafana_ip:3000/login` | `admin` / `admin` |
| **Kibana** | `http://kibana_ip:5601` | – (без аутентификации) |
>Важно:
...
- Отказоустойчивость: две веб‑ВМ в разных зонах доступности + ALB с Health Check. Если одна ВМ упадёт, трафик уйдёт на вторую.

- Мониторинг: Prometheus + Node Exporter + Nginx Log Exporter дают представление о состоянии серверов и качестве обслуживания HTTP‑запросов.

- Логирование: Filebeat → Elasticsearch + Kibana — это стандартный и понятный стек для анализа логов.

- Бэкапы: двухуровневая схема (локальные rsync‑бэкапы на Bastion + архив в Object Storage) позволяет быстро восстановить файлы и иметь долгосрочную копию в облаке.

- Безопасность: приватные подсети, ограничение доступа к публичным портам по IP, использование сервисного аккаунта вместо ключей пользователя.

- Экономичность: прерываемые ВМ и IaC позволяют многократно поднимать и удалять стенд без существенных затрат.
