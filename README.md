...
| `internal` | 22 (SSH), ANY (внутренний трафик) | `10.0.0.0/16` | Взаимодействие между ВМ |
| `web` | 80 (HTTP от ALB), 9100 (Node Exporter), 4040 (Nginx Log Exporter) | `10.0.0.0/16` | Доступ к веб-серверам и экспортёрам |

> **Важно:** все публичные порты на Bastion ограничены моим IP-адресом для повышения безопасности.
...
При проектировании я старался придерживаться «облачного» подхода, но с учётом того, что это учебный проект, некоторые решения были заменены на более удобные для демонстрации.

**Балансировку сделал через ALB, а не через Nginx на бастионе.**
Изначально в задании балансировка не была строго регламентирована, но преподаватель указал на уязвимость единой точки отказа при использовании Nginx на Bastion. Вместо того чтобы городить костыли, я переделал на Yandex Application Load Balancer — он и сам мониторит здоровье, и трафик распределяет автоматически, и не требует лишнего обслуживания.

**Bastion оставил только для SSH и прокси к Grafana с Kibana.**
Веб-трафик теперь идёт через ALB, так что бастион не перегружается. Из публичных портов на нём торчат только 22, 3000 и 5601, причём доступ к ним ограничен моим IP — чтобы не светить админки на весь мир.

**Бэкапы сделал двухуровневыми.**
По заданию требовались снапшоты дисков, но на практике в Yandex Cloud они стоят денег и не всегда удобны для частого восстановления отдельных файлов. Поэтому я оставил локальные копии на Bastion для быстрого доступа, а сверху добавил выгрузку в Object Storage — это надёжнее и полностью автоматизируется. Преподаватель такой вариант одобрил.

**Все сервисы завернул в Docker.**
Это не было обязательным, но сильно упростило развёртывание и переносимость. Не надо гадать, какая версия пакета стоит на ВМ, — достаточно перезапустить контейнер.

Конечно, я понимаю, что в промышленной среде некоторые вещи стоило бы реализовать иначе. Но поскольку это курсовая, я старался найти баланс между «как правильно» и «как проще понять и показать», постоянно сверяясь с замечаниями преподавателя.
...
- **Отказоустойчивость** – две веб‑ВМ в разных зонах доступности + ALB с Health Check (замена ручной балансировки на Bastion).
